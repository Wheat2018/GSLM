...
def print_func_params(local_variables: dict, max_char=100):
    print(inspect.stack()[1][3], end='(\n\t')

    def to_string(elem):
        if isinstance(elem, list):
            return str([to_string(x) for x in elem])
        elif isinstance(elem, dict):
            return str({to_string(k): to_string(v) for k, v in elem.items()})
        else:
            string = str(elem).replace('\n', '\\n')
            if max_char > 0 and len(string) > max_char:
                string = string[: max_char - 3] + '...'
            return string

    print(', \n\t'.join([f'{to_string(k)} = {to_string(v)}' for k, v in local_variables.items()]))
    print(')')