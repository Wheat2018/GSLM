...
def random_resize_long(img, min_long, max_long, order=3):
    target_long = random.randint(min_long, max_long)
    if isinstance(img, tuple):
        assert len(img) > 0 and all([x.shape[:2] == img[0].shape[:2] for x in img])
    else:
        img = (img,)
    ...
    res = [pil_rescale(i, scale, o) for i, o in zip(img, order)]
...
# g_coms = [1, 3, 5, 10]
# b_sxys = [5, 20, 80, 100]
# b_rgbs = [1, 5, 13, 20, 100]
# b_coms = [1, 5, 10, 20, 100]
def crf_inference_label(img, labels, t=10, n_labels=21, gt_prob=0.7, g_sxy=3, g_com=3, b_sxy=50, b_rgb=5, b_com=10):
    ...
    d.addPairwiseGaussian(sxy=g_sxy, compat=g_com)
    d.addPairwiseBilateral(sxy=b_sxy, srgb=b_rgb, rgbim=np.ascontiguousarray(np.copy(img)), compat=b_com)
