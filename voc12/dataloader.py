...
from misc.visualizeutils import read_palette_image
...
def load_img_name_list(dataset_path):
    ...
    img_name_list = np.loadtxt(dataset_path, dtype=str).astype(np.int32)
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,
        ...
        super().__init__(img_name_list_path, voc12_root, img_normal=img_normal, resize_long=resize_long, 
                         rescale=rescale, hor_flip=hor_flip, crop_size=crop_size, crop_method=crop_method)
        self.scales = scales
    ...
    def __getitem__(self, idx):
        ...
        ms_img_list = []
        ori_img = torch.from_numpy(img.copy())
        ...
        out = {"name": name_str, "img": ms_img_list, "size": (img.shape[0], img.shape[1]),
               "label": torch.from_numpy(self.label_list[idx]), "ori_img": ori_img}
        return out

class VOC12SegmentationDataset(Dataset):

    def __init__(self, img_name_list_path, crop_size, voc12_root, label_dir=None,
                 rescale=None, img_normal=TorchvisionNormalize(), hor_flip=False,
                 crop_method = 'random', resize_long=None):

        self.img_name_list = load_img_name_list(img_name_list_path)
        self.label_list = load_image_label_list_from_npy(self.img_name_list)
        self.voc12_root = voc12_root

        self.label_dir = label_dir or os.path.join(voc12_root, SEG_FOLDER_NAME)
    ...
    def __getitem__(self, idx):
        ...
        img = imageio.imread(get_img_path(name_str, self.voc12_root))
        label = read_palette_image(os.path.join(self.label_dir, name_str + '.png'))

        img = np.asarray(img)

        if self.resize_long:
            img, label = imutils.random_resize_long((img, label), self.resize_long[0], self.resize_long[1], order=(3, 0))
        ...
        return {'name': name_str, 'img': img, 'label': label, 'cls': torch.from_numpy(self.label_list[idx])}
...
class VOC12AffinityDataset(VOC12SegmentationDataset):
    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root,
        ...
        super().__init__(img_name_list_path, crop_size, voc12_root, label_dir, rescale, img_normal, hor_flip, crop_method=crop_method)
